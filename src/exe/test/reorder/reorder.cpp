...
auto counter_gauss_sum_gpu = std::transform_reduce(std::begin(count_gpu),
std::end(count_gpu), (GLuint)0, std::plus<>(), [](const auto& n) { if (n == 0) {
          return (GLuint)0;
      } else {
...
auto elem_sum_gpu = std::transform_reduce(std::begin(m2), std::end(m2), 0,
std::plus<>(),
  [](const auto& elem) { return elem; });
