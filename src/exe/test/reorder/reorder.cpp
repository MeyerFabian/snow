int main()
{
    ...
    std::vector<Scan> particles;
    std::vector<GLuint> grid_offset;
    for (size_t i = 0; i < numVectors; i++) {
	particles.push_back({

	    PREC_VEC_TYPE(
		glm::linearRand(0.0f, 1.0f), glm::linearRand(0.0f, 1.0f), glm::linearRand(0.0f, 1.0f), 0.0f)
    ...
    auto counter_gauss_sum_gpu = std::transform_reduce(std::begin(count_gpu), std::end(count_gpu),
    ...
    auto elem_sum_gpu = std::transform_reduce(std::begin(m2), std::end(m2), 0, std::plus<>(),
	[](const auto& elem) { return elem; });
