/*
#include "P2GPushSyncPipeline.hpp"

void P2GPushSyncPipeline::init(
    P2GPushSyncTechnique::P2GPushSyncData&& pipeline_data, IOBufferData&& io) {
  local_size = pipeline_data.local_size;

  P2GPushSyncTechnique::P2GPushSyncData reduction_data_2nd = pipeline_data;
  ...
  P2GPushSyncTechnique::P2GPushSyncData reduction_data_alt = pipeline_data;
  reduction_data_2nd.local_size = {1, 1, 1};
...
void P2GPushSyncPipeline::run(GLuint numVectors) {
  if (numVectors > 1024) {
    BenchmarkerGPU::getInstance().time(
        "P2GPushSyncPipeline Map+Reduce", [this, numVectors]() {
    ...
    std::string log_name = "P2GPushSyncPipeline Reduce: (" +
    ...
    std::string log_name = "P2GPushSyncPipeline altStep: (" +
                           std::to_string(numVectors) + " -> 1)";
...
void P2GPushSyncPipeline::runNoSeqAdd(GLuint numVectors) {
  buffer_size_after = numVectors / local_size.x;
  BenchmarkerGPU::getInstance().time(
      "P2GPushSyncPipeline Map+Reduce",
