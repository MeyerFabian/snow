...
void main(void){
	...
	for(int pow_d_of_2 = X; pow_d_of_2 > 0; pow_d_of_2 >>= 1) {
		...
		if (tIndex < pow_d_of_2) {
			...
			// if leftSharedIndex is 2**(d-1)-1
			// rightSharedIndex is 2**d-1
	...
	if(tIndex==0) { s_data[X*2-1] = BINARY_OP_NEUTRAL_ELEMENT;}

	//down-sweep, reverse parallel red. take more threads each time
