...
void main(void){
	...
	if(inBounds(globalGridIndex,gGridDim)){
		...
		float voxelAndTileIndexFloat = float(voxelAndTileIndex);
		float scanIndexFloat  = float(scanIndex);
		float scanOffsetFloat = float(scanOffset);
		float unsortedIndexFloat = float(unsortedIndex);
		OUTPUT_AT(OUTPUT,0,OUTPUT_SIZE,sortedIndex,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER,OUTPUT_VAR_SIZE) =vec4(unsortedIndexFloat,scanIndexFloat,scanOffsetFloat,voxelAndTileIndexFloat);
