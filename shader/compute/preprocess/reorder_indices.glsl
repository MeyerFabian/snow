...
void main(void){
	...
	if(inBounds(globalGridIndex,grid_def.gGridDim)){
		uint voxelAndTileIndex = get_voxel_and_tile_index(globalGridIndex,grid_def.gGridDim);
		...
		OUTPUT_AT(OUTPUT,OUTPUT_VAR,OUTPUT_SIZE,sortedIndex,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER,OUTPUT_VAR_SIZE) = unsortedIndex;
	}
}
