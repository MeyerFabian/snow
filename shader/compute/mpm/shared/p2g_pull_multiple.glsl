...
/*
 * p2g_pull_simple.glsl is without batching, easier to read at first. Batching handles multiple particles at once.
 *
 * THIS METHOD WAS ALWAYS SLOWER THAN SIMPLE in all tests i did
 *
...
void main(void){
	...
	for(int particle_count = 0; particle_count < tile_count; particle_count+=MULTIPLE_PARTICLES){
		...
		for(int local_i = int(gl_LocalInvocationID.x); local_i < HALO_FLAT;local_i += X*Y*Z) {
			...
			//ignore optimization cases(unrolling) for first read and go to #else, line 147
