		...
		for(int y = -LEFT_SUPPORT; y<= RIGHT_SUPPORT ;y++){
			for(int z = -LEFT_SUPPORT; z <= RIGHT_SUPPORT ;z++){
				...
				if(inBounds(globalGridIndex,grid_def.gGridDim)){
					...
					float mi = mp*wip;
					vec3 vi = vp*mp*wip;
					atomicAdd(OUTPUT_AT(OUTPUT,Gridpoint_vel_mass,OUTPUT_SIZE,key,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER).w,
							mi);
					atomicAdd(OUTPUT_AT(OUTPUT,Gridpoint_vel_mass,OUTPUT_SIZE,key,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER).x,
							vi.x);

					atomicAdd(OUTPUT_AT(OUTPUT,Gridpoint_vel_mass,OUTPUT_SIZE,key,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER).y,
							vi.y);

					atomicAdd(OUTPUT_AT(OUTPUT,Gridpoint_vel_mass,OUTPUT_SIZE,key,OUTPUT_NUM_BUFFER,OUTPUT_INDEX_BUFFER).z,
							vi.z);
				}
			}
		}
